	...
	<view>
		...
		<view class="home-menu display-between-center">
			<view class="menu-item" v-for="(item,key) in menus" :key="key" @click="goPage(item.url)">
				<view v-if="key!==5 || isGmtInviteUser===0" class="menu-icon"><image :src="item.icon" style="height: 100%;"></image></view>
				<view v-if="key!==5 || isGmtInviteUser===0" class="menu-text">{{item.name}}</view>
			</view>
		</view>
		<!-- 提示框 -->
		...
		<my-modal ref="modal" :msgText="msgText" @confirmAction="agreeFollow" confirmText="确认"></my-modal>
		<!-- 手机号授权  -->
...
<script>
	...
	export default {
		...
		computed:{
			...
			isGmtInviteUser() {
			  return this.$store.state.person.isGmtInviteUser || 0;
			},
		},
		watch:{
			// 当获取到token时触发某些事件
		...
		methods: {
			...
			agreeFollow(){
				if(this.isLogin){
					...
					this.myRequest("/gmt/api/userFollow/bundling",{data}).then(res =>{ 
						console.log(res)
						this.$refs.loading.hideLoading();
						if(res.data.code===0){
							this.getChildren();
							this.toastTitle = "关注成功";
							this.toastType = "ring";
							this.$refs.toast.showLoading();
						}else{
							this.toastTitle = "关注失败";
